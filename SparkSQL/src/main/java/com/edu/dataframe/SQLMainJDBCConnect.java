...
/**
 * Class to process the sql data base using the spark SQL framework
 * 
 * How to run:
 * Before runing program give command line argument in argument section
 * as follows:
 * 
 * src/main/resources/MetaModel.json src/main/resources/DataFrameGeneration
 * 
 *  @author mandar
 * 
 */
public class SQLMainJDBCConnect implements Serializable {

	private static final org.apache.log4j.Logger LOGGER = org.apache.log4j.Logger
			.getLogger(SQLMainJDBCConnect.class);
	...
	public static void main(String[] args) {
		...
		DataFrameMetaDataModel dataFrameModel = MetaDataGenerator
				.generateMetaDataModel(columnType);

		ObjectMapper objectMapper = new ObjectMapper();

		try {
			objectMapper.writeValue(new File(args[0]),
					dataFrameModel.getMetaDataList());
		...
		System.out
				.println("-------Data frame Generation by using JSON file------");
		DataFrame generateDataFrame = MetaDataGenerator.generateDataFrame(
				args[0], sqlContext);
		printDataFrameScheme(generateDataFrame);
		System.out
				.println("------------Print the element of the data frame---------");
		displayDataFrame(generateDataFrame.collectAsList());

		System.out
				.println("-------Delete the column name from the meta-data data frame");
		DataFrame deleteColumnDF = generateDataFrame.drop("distType");
		printDataFrameScheme(deleteColumnDF);
		displayDataFrame(deleteColumnDF.collectAsList());

		// write the data frame to parqute file

		generateDataFrame.write().parquet(args[1]);
		System.out.println("Data frame write to parquet file successfully");

		// Read the data frame from parquet file
		DataFrame readFromParquteFile = sqlContext.read().parquet(args[1]);

		printDataFrameScheme(readFromParquteFile);
		displayDataFrame(readFromParquteFile.collectAsList());
	...
	private static void printDataFrameScheme(DataFrame df) {
		...
		Map<String, String> columnElement = new HashMap<>();
